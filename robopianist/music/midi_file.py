...
class NoteTrajectory:
    ...
    def get_min_max_key(self):
        ...
        for note in self.notes:
            for n in note:
                min_key = min(min_key, n.key)
                max_key = max(max_key, n.key)
        return min_key, max_key
    
    def shift(self, offset_scale:int):
        """Shifts the note trajectory by the given offset."""
        if offset_scale == 0:
            return self
        min_key, max_key = self.get_min_max_key()
        min_offset = -min_key
        max_offset = consts.MAX_KEY_NUMBER - max_key
        offset = offset_scale * 12
        if offset < min_offset or offset > max_offset:
            raise ValueError(
                f"Offset must be in [{min_offset}, {max_offset}], got {offset}."
            )
        for i, note in enumerate(self.notes):
            for j, n in enumerate(note):
                object.__setattr__(self.notes[i][j], 'key', n.key + offset)
        return self
